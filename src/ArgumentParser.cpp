...
void flag_a(UserOptions& options) { options.showHiddenFiles = true; }
...
void flag_G(UserOptions& options) { options.longListOptions.showOwnerGroup = false; }
...
std::unordered_map<std::string, std::function<void(UserOptions&)>> getFlagActions()
{
  ...
  const static std::unordered_map<std::string, std::function<void(UserOptions&)>> flagActions{
    { "a", flag_a }, { "all", flag_a }, { "l", flag_l }, { "g", flag_g }, { "o", flag_o }, { "G", flag_G }, { "no-group", flag_G }
  };
  return flagActions;
}
...
std::vector<std::string> extractFeatureFlags(std::span<const char*> args)
{
  ...
  for (const std::string& arg : args.subspan(1)) {
    if (arg.starts_with("--")) {
      if (!allowedFlags.contains(arg.substr(2))) { throw std::invalid_argument("Invalid flag: " + arg); }
      enabledFlags.emplace(arg.substr(2));
    } else {
      for (const char flag : arg.substr(1)) {
        if (!allowedFlags.contains(std::string(1, flag))) { throw std::invalid_argument("Invalid flag: " + std::string{ flag }); }
...
UserOptions parseArgs(std::span<const char*> args)
{
  ...
  for (const auto& arg : extractFeatureFlags(args)) { flagActions[arg](opts); }
  return opts;
}
